    ...
    <div class="container mx-auto px-4">
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST" class="max-w-lg">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" class="w-full rounded border-gray-300" required>
                @error('name')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" class="w-full rounded border-gray-300">
                @error('slug')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="w-full rounded border-gray-300">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
